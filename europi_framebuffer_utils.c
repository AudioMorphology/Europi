/*
* EUROPI_FRAMEBUFFER_UTILS.C
*
* Collection of various utilities to write graphics
* to the raspberry Pi Framebuffer. Note that I decided
* it was best to stick with the default 16-Bit 565 format
* of graphics on small screensx attached to the Pi, so
* although there are functions here to write 8-Bit graphics,
* Europi doesn't actually make use of them.
* 
*/
#include <linux/fb.h>
 ...
 */
void put_char(char *fbp, int x, int y, int c, int colidx)
{
	int i,j,bits;
	...
	bits = font_vga_8x16.data [font_vga_8x16.height * c + i];
		for (j = 0; j < font_vga_8x16.width; j++, bits <<= 1)
			if (bits & 0x80){
				put_pixel_RGB565(fbp, x+j, y+i, HEX2565(0x000000));
			}
		}
}
/* put_string
 * 
 * Csalls put_char multiple times to write a string
 * to the screen using an included 8x16 font
 */
void put_string(char *fbp, int x, int y, char *s, unsigned colidx)
{
	int i;
	for (i = 0; *s; i++, x += font_vga_8x16.width, s++)
	put_char (fbp, x, y, *s, colidx);
}